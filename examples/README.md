...
这个目录只包含**人工选择并审核后公开的示例**，不是维护者的全部历史，也不会自动接收未来生成的客户图片。

v0.2 增加了 [FLUX 中英文指令配对示例](flux-language-check/README.md)，包含 Kontext、FLUX.2 Pro 和 FLUX.2 Flex 的原图、实际发送的提示词和英文准备记录。下面保留的是原有中文书店海报案例。
...
这份早期海报记录没有 GPT Image 2.5 图片，因为采集时该槽位尚未配置。v0.3 已把 Flare 与 Sunburst 作为独立模型接入，但不会回填或改写这份历史实验。公开示例不包含人工评分或私人评语，避免把未经统一标准的人评数字呈现为客观质量指标。
